...
export class ToastService {
  ...
  public addInfoToast(
    options: { header: string; message: string },
    stack: boolean = true
  ) {
    this.addToast(
      {
  ...
  private addToast(
    options: {
      ...
      header: string;
      color?: 'tertiary' | 'danger' | 'warning' | 'success';
    },
    stack: boolean
  ) {
    ...
    const colorToIcon = {
      tertiary: '',
      danger:
        '<ion-icon name="close-circle-outline" color="danger" style="font-size: 1.5rem"></ion-icon>',
      warning:
        '<ion-icon name="alert-circle-outline" color="warning" style="font-size: 1.5rem"></ion-icon>',
      success:
